...
A personal CLI that joins coding-agent session logs — Claude Code's and Codex's — with git repo state to produce a morning triage list: what changed, what still needs a decision, and which session of which agent did it.
...
**Session**:
One conversation with a coding agent, identified by its JSONL file — a Claude Code log (`~/.claude/projects/<slug>/<sessionId>.jsonl`, carrying a native title: `aiTitle`/`customTitle`/`slug`) or a Codex rollout (`~/.codex/sessions/YYYY/MM/DD/rollout-<ts>-<id>.jsonl`, carrying no title, so its first prompt stands in). The two are read by two **dialect** modules into one typed reading, and every view consumes the reading, never the schema (ADR 0001 § two dialects, one reading). A Session's **agent** (`claude` | `codex`) is a fact read off the file's shape, printed as a bare dim tag after the title wherever a Codex Session is named; Claude Code Sessions carry no tag. Codex's own threads beside yours — its auto-reviewer's `subagent` and `guardian_review` rollouts — are not Sessions. A Codex thread that is resumed writes a second rollout, and each rollout is its own Session, addressed by the id unique to its file name.
_Avoid_: conversation, chat, run, thread (Codex's word for the same thing — a rollout *file* is what Standup reads, and a resumed thread is two of them)

**Scan Universe**:
The set of repos Standup inspects — auto-discovered from the `cwd` fields inside the **Session** logs under its two roots, `~/.claude/projects` and `~/.codex`; never configured by hand, never walked from disk roots. Either root may be absent (one agent installed); the Universe reads the ones that exist and errors only when neither is there (ADR 0001 § two dialects, one reading).
...
**Call**:
One **Feed Event** for a tool call that changes no file — a shell command, an MCP request, a web fetch, a subagent spawn, anything the agent invokes that the feed does not already narrate as a file change. Carries the tool's display name, as much of its input as the row can hold — the most readable key's value first, the remaining keys after it as compact JSON — and the `✓`/`✗` verdict patched on when the result lands. **The shell tool is a Call** — Claude Code's `Bash`, Codex's `exec_command` — and the only one whose argument is a shell command, so it alone renders `$ …` with shell lexing. Expanded, a Call shows its input **entire**: every key by path, unclipped, with the line structure the log recorded, so the request the agent actually made is readable rather than summarised. The Watch shows what was *asked*, never what came back — on the header and in the expanded body alike: a result's content belongs to the **Transcript**, which renders the same tool call through the same header renderer, and under `--tools` through the same whole-input renderer, so the two surfaces cannot drift. **Local reads are silent** (Claude Code's `Read`, `Grep`, `Glob`, `NotebookRead`, `BashOutput`, `KillShell`; Codex reads through its shell, so nothing of its is silent) — they change nothing and the **Activity State**'s `reading` already answers for them; every other tool earns a row, *including one that does not exist yet*, the same call each dialect's `ACT_VERBS` makes when an unmapped tool falls to `acting` (ADR 0004 § Calls). Not folded: ten fetches are ten rows, because unlike a **Change Run**'s hunks they are ten different acts with ten different arguments. A tool whose server the log names only as a UUID is shown by its bare tool name — Standup will not print an identifier that names nothing to the reader.
...
is all there is to read. An **interrupt** (`Esc`, the session quitting
mid-turn, or a refused tool call) is a *fact* in the log and settles the state;
without reading it, `thinking` would be claimed forever. A tool verb carries a **display floor** —
