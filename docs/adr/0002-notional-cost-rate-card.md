...
- **Not a Session.** It has no title lines, no `cwd` discovery role, no Session
  Handle — `standup session` cannot address it. A row that cannot be drilled
  into would be a dead end in the one view built for drilling.
