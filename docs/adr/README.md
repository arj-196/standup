...
| # | Decision | Covers |
|---|---|---|
| [0001](0001-what-standup-reads-and-what-it-keeps.md) | What Standup reads, what it keeps, and what time means | Scan Universe (two roots); retired Checkpoint and the Recent Window; Derived Cache and the durable-root split; the accelerator protocol its artifacts are declared through; the one log reader and what `last_activity` means; one module owns the scan (Repo Entry identity, cache lifecycle); `--since` as the only knob; ageless attribution; two dialects, one reading (Claude Code and Codex logs, the `logs` seam, `Session.agent`) |
| [0002](0002-notional-cost-rate-card.md) | Notional Cost from a single dated Rate Card | why no Real Spend is reported at all; subagent usage folds into the parent Session; Codex usage (OpenAI rows, uncached input) |
| [0003](0003-out-of-band-artifacts.md) | Out-of-Band Artifacts | the model both share; the one Artifact store behind them (tolerance, clock, naive timestamps); the one LLM-pass seam (Pass/Result, empty-result and timeout rules, transport choice, the shared digest); the Session Brief and its Stop hook; the Audit, Loops, and the fixed Expert Panel |
| [0004](0004-the-watch.md) | The Watch | event source; stream/UI boundary and the row model on its Textual-free side; discovery vs the constructor (and the types the UI reads); motion never outliving data; Activity State; Change Runs; removed-row field; fold vs clip; selection in the lane; mouse gestures |
